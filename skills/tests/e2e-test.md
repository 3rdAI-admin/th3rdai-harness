---
description: End-to-end testing with two modes. Mode A drives a real browser (Vercel Agent Browser CLI) to test every user journey of a runnable app — screenshots, UI/UX validation, and database verification. Mode B evaluates a harness artifact (agent, prompt version, skill, model profile, or config) against an eval case and rubric, recording results to evals/results/ and runs/. Run after implementation to validate before review or release.
---

# End-to-End Testing

This skill implements the testing side of the harness (Stage 05: Evaluation) and supports two modes. Pick the mode that matches what you are validating.

## Harness References

- Evaluator Agent: `agents/evaluator.agent.md`
- Rubrics: `evals/rubrics/` · Eval cases: `evals/cases/` · Results: `evals/results/`
...
- **Mode A — Application E2E (browser):** Use when the harness has produced a runnable app/UI to exercise end-to-end. This is the browser-driven flow in **Pre-flight Check** through **Report** below.
- **Mode B — Harness artifact testing:** Use when validating a harness artifact (agent, prompt version, skill, model profile, or config) against an eval case and rubric. No browser or frontend required. See **Mode B** immediately below.

## Mode B: Harness Artifact Testing
...
### B2. Run the evaluation
1. Produce the artifact's output for the eval case input.
2. Score each rubric criterion (1-5) and document the rationale — do not change the rubric mid-run.
3. Compare against the rubric's pass threshold.
4. **Prompt/config changes:** also run the prior version on the same case and compare for regressions. When comparing model profiles, use `evals/results/_TEMPLATE-model-comparison.md`.

### B3. Record results
1. Write an eval result to `evals/results/<YYYYMMDD>-<case>.md`.
2. Write a run record to `runs/<run-id>.md` per the run-log schema.
3. If a blocking criterion fails, do **not** mark the artifact release-ready — hand off to Stage 06 (Iteration).

### B4. Report
Summarize: artifact tested, case + rubric, per-criterion scores, pass/fail, lowest-scoring criteria, any regressions, and the recommended next stage.

---

The remainder of this document is **Mode A (Application E2E)**.
...
If the command is not found, ask the user for approval before installing it:
```bash
npm install -g agent-browser
```

After installation (or if it was already installed), ask before ensuring the browser engine is set up:
```bash
...
1. Ask before installing dependencies if needed
2. Start the dev server **in the background** using the shell's background-process mechanism or the IDE's long-running command support
...
When the app under test is a harness-built artifact, also write a run record to `runs/<run-id>.md` per `telemetry/run-log-schema.md` (inputs, outputs, validation status, issues, follow-up) so the test is traceable alongside other harness runs — not only in `e2e-screenshots/`.
